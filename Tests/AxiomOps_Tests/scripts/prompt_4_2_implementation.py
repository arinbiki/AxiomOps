#!/usr/bin/env python3
"""
Prompt 4.2 — Test 4.2 Duplicate Prevention

This script implements the exact requirements from COPILOT_PROMPTS.md Prompt 4.2:
- Test duplicate prevention for finding ingestion
- Send same finding twice
- Verify only one evidence record created
...
3. Send finding_failed_high.json via POST to http://localhost:5678/webhook/complianceops-finding-ingestion
4. Send same finding again via POST to http://localhost:5678/webhook/complianceops-finding-ingestion
5. Wait 3 seconds
6. Query Postgres: SELECT COUNT(*) FROM evidence_events WHERE run_id = 'RUN-TEST-001';
7. Query Postgres: SELECT occurrence_count FROM findings WHERE finding_key = 'CC6.6::arn:aws:s3:::prod-data-lake::SOC2';

EXPECTED RESULTS:
- evidence_events table has 1 row (duplicate prevented)
- findings table has 1 row with occurrence_count = 1 (not incremented)

EXPECTED OUTPUT FORMAT:
{
  "test_id": "4.2",
  "test_name": "duplicate_prevention",
  "status": "PASS or FAIL",
  "evidence_rows": 0,
  "occurrence_count": 0,
  "notes": ""
...
# ==================== MAIN IMPLEMENTATION ====================
def main():
    """Execute the exact steps from Prompt 4.2"""
    
    print("=" * 60)
    print("ROLE: Unit Test Executor for Workflow 4.2")
    print("TASK: Test duplicate prevention for finding ingestion.")
    print("=" * 60)
    print()
    ...
    # Step 3: Send finding_failed_high.json via POST to http://localhost:5678/webhook/complianceops-finding-ingestion
    print("3. Send finding_failed_high.json via POST to http://localhost:5678/webhook/complianceops-finding-ingestion")
    
    payload = load_fixture("finding_failed_high.json")
    resp1 = post_webhook("complianceops-finding-ingestion", payload,
                         headers={"X-COMPLIANCEOPS-TOKEN": INGEST_TOKEN})
    print(f"   ✅ First send: HTTP status {resp1['status']}")
    
    # Step 4: Send same finding again via POST to http://localhost:5678/webhook/complianceops-finding-ingestion
    print("4. Send same finding again via POST to http://localhost:5678/webhook/complianceops-finding-ingestion")
    
    resp2 = post_webhook("complianceops-finding-ingestion", payload,
                         headers={"X-COMPLIANCEOPS-TOKEN": INGEST_TOKEN})
    print(f"   ✅ Second send: HTTP status {resp2['status']}")
    
    # Step 5: Wait 3 seconds
    print("5. Wait 3 seconds.")
    time.sleep(3)
    
    # Step 6: Query Postgres: SELECT COUNT(*) FROM evidence_events WHERE run_id = 'RUN-TEST-001';
    print("6. Query Postgres: SELECT COUNT(*) FROM evidence_events WHERE run_id = 'RUN-TEST-001';")
    ...
    # Step 7: Query Postgres: SELECT occurrence_count FROM findings WHERE finding_key = 'CC6.6::arn:aws:s3:::prod-data-lake::SOC2';
    print("7. Query Postgres: SELECT occurrence_count FROM findings WHERE finding_key = 'CC6.6::arn:aws:s3:::prod-data-lake::SOC2';")
    ...
    try:
        ...
        error_result = {
            ...
            "test_name": "duplicate_prevention",
            "status": "FAIL",
            "evidence_rows": evidence_result[0] if evidence_result else 0,
            "occurrence_count": 0,
    ...
    # Extract results
    evidence_rows = evidence_result[0]
    occurrence_count = finding_result[0]
    
    # Determine status
    status = "PASS" if (evidence_rows == 1 and occurrence_count == 1) else "FAIL"
    ...
    print(f"\n📊 Test Results:")
    print(f"   - Evidence rows: {evidence_rows} (expected: 1)")
...
if __name__ == "__main__":
    print("🚀 Starting Prompt 4.2 Implementation")
